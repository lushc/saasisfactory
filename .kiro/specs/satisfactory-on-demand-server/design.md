...
1. **Authentication Flow**:
   ...
   - Control Lambda validates password against Parameter Store
...
2. **Server Start Flow**:
   ...
   - If server is unclaimed (first startup):
     ...
     - Stores admin password and Admin token in Parameter Store
   - If server is already claimed (subsequent startups):
     - Control Lambda retrieves admin password from Parameter Store
     - Calls PasswordLogin to get Admin token
     - Stores Admin token in Parameter Store
...
3. **Server Monitoring Flow**:
   ...
   - Monitor Lambda retrieves Admin token from Parameter Store
   - Monitor Lambda verifies token with VerifyAuthenticationToken
   - If token invalid, Monitor Lambda regenerates token via PasswordLogin and updates Parameter Store
...
4. **Server Stop Flow**:
   ...
   - Control Lambda retrieves Admin token from Parameter Store
   - Control Lambda verifies token with VerifyAuthenticationToken
   - If token invalid, Control Lambda regenerates token via PasswordLogin and updates Parameter Store
...
- Parameter Store parameters (created with placeholder values, populated by post-deploy script):
...
- Configuration values stored in AWS Parameter Store with hierarchical naming for easy retrieval
- CloudFormation creates parameter placeholders, post-deploy script populates values
...
2. Verify JWT signature using secret key from Parameter Store
...
**POST /auth/login**
- Validates admin password against Parameter Store
...
- Claims server if unclaimed (first startup):
  ...
  - Stores password in Parameter Store
  - Calls PasswordlessLogin to get InitialAdmin token
  - Calls ClaimServer with admin password
  - Stores returned Admin token in Parameter Store
- If server already claimed (subsequent startups):
  - Retrieves admin password from Parameter Store
  - Calls PasswordLogin to get Admin token
  - Stores token in Parameter Store
...
**POST /server/stop**
- Retrieves Admin token from Parameter Store
...
- Retrieves Admin token from Parameter Store (for Satisfactory Server)
- Verifies token with VerifyAuthenticationToken
- If token invalid, regenerates token via PasswordLogin
- Retrieves client protection password from Parameter Store
...
- Updates client protection password in Parameter Store
- Returns success status
- **Requires authentication**
...
3. Retrieve Admin token from Parameter Store
4. Verify token with VerifyAuthenticationToken
5. If token invalid, regenerate token via PasswordLogin and update Parameter Store
...
4. **Store Token** - Save Admin token in Parameter Store for future API calls
...
- If verification fails (401 response or error):
  - Retrieve admin password from Parameter Store
  - Call PasswordLogin to get new Admin token
  - Update token in Parameter Store
...
- Parameter Store calls: Exponential backoff, max 3 retries
...
- Parameter Store: 10,000 parameters/month (first 12 months)
...
5. **Parameter Store**:
   - 5 parameters × $0.00/month = $0.00 (within free tier)
   - **Subtotal**: $0.00
...
2. **Start Server and Automatic Claiming**:
   ...
   - Control Lambda automatically claims server on first startup:
     ...
     - Stores password and token in Parameter Store
...
- **Password Storage**: Admin password stored securely in AWS Parameter Store, never in code or configuration files
- **API Gateway Authorization**: Lambda authorizer validates JWT signature and expiration for all protected endpoints
- **Satisfactory Server API Token**: Stored in Parameter Store, never exposed to frontend
...
- **Parameter Store Encryption**: All parameters encrypted at rest with automatic key rotation support
...
- Parameter Store: GetParameter, PutParameter (specific parameters only)
...
- Parameter Store: GetParameter (Satisfactory Admin token only)
- CloudWatch Logs: CreateLogGroup, CreateLogStream, PutLogEvents

**Authorizer Lambda Role**:
- Parameter Store: GetParameter (JWT secret and admin password only)
- CloudWatch Logs: CreateLogGroup, CreateLogStream, PutLogEvents

**ECS Task Execution Role**:
- ECR: GetAuthorizationToken, BatchCheckLayerAvailability, GetDownloadUrlForLayer, BatchGetImage
- CloudWatch Logs: CreateLogStream, PutLogEvents
- Parameter Store: GetParameter (if parameters passed as environment variables)
...
- **Parameter Access Logging**: CloudTrail logs all Parameter Store access
- **No Hardcoded Configuration**: All configuration values retrieved from Parameter Store at runtime
- **Environment Variables**: Avoid passing sensitive parameters as environment variables where possible
...
5. **Run Post-Deployment Script**:
   ...
   This script will:
   - Check if admin password and JWT secret already exist in Parameter Store
...
if aws ssm get-parameter --name "/satisfactory/admin-password" 2>/dev/null; then
  ...
  aws ssm put-parameter \
    ...
    --type "SecureString" \
    --overwrite
  echo "Admin password generated and stored in Parameter Store"
fi

# Check if JWT secret already exists
if aws ssm get-parameter --name "/satisfactory/jwt-secret" 2>/dev/null; then
  ...
  aws ssm put-parameter \
    ...
    --type "SecureString" \
    --overwrite
  echo "JWT secret generated and stored in Parameter Store"
fi
