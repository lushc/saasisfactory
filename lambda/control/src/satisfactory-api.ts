...
export class SatisfactoryApiClient {
  ...
  /**
   * Call PasswordlessLogin to get InitialAdmin token (for unclaimed servers)
   */
  async passwordlessLogin(): Promise<string> {
  ...
  /**
   * Claim the server with admin password (first-time setup)
   */
  async claimServer(initialAdminToken: string, adminPassword: string): Promise<string> {
    const response = await this.client.post<SatisfactoryApiResponse<AuthTokenResponse>>('', {
      ...
      headers: {
        Authorization: `Bearer ${initialAdminToken}`
      }
    });
  ...
  /**
   * Login with admin password (subsequent startups)
   */
  ...
  /**
   * Set client protection password
   */
  async setClientPassword(token: string, password: string): Promise<void> {
    await this.client.post('', {
